...
import { traceLoading } from './loadingTrace';
...
export async function fileExistsNotHtml(url: string): Promise<boolean> {
  try {
    traceLoading('file-exists:head:start', { url });
    const response = await fetch(url, { method: 'HEAD' });
    traceLoading('file-exists:head:end', {
      url,
    ...
    traceLoading('file-exists:head:error', { url });
    return false;
  }
}
...
export async function checkFileExists(url: string): Promise<boolean> {
  ...
  if (manifestAvailability !== null) {
    traceLoading('file-exists:manifest', {
      url,
      exists: manifestAvailability,
    });
  ...
  try {
    traceLoading('file-exists:range:start', { url });
    const response = await fetch(url, {
      method: 'GET',
      headers: { Range: 'bytes=0-0' },
    });
    traceLoading('file-exists:range:end', {
      url,
      status: response.status,
    ...
    traceLoading('file-exists:range:error', { url });
    return false;
  }
}
