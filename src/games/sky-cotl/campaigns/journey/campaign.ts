import {
  baseCenteredIconClass,
  getCampaignIconSizeClass,
  type Campaign,
  type CampaignIconProps,
} from '@engine/types';
import { jsx, jsxs } from 'react/jsx-runtime';
import { skyCotlCampaignStrings } from '../../strings';
import { questions } from './questions';
import { theme } from './theme';

const SkyJourneyIcon = ({ className, size }: CampaignIconProps) =>
  jsx('div', {
    className: `${baseCenteredIconClass} ${className ?? getCampaignIconSizeClass(size)}`,
    'aria-label': skyCotlCampaignStrings.journey.iconAriaLabel,
    children: jsxs('svg', {
      viewBox: '0 0 56 56',
      className: 'w-full h-full',
      role: 'img',
      'aria-hidden': true,
      style: { filter: 'drop-shadow(0 0 10px rgba(56,189,248,0.55))' },
      children: [
        jsx('path', {
          d: 'M18 34c-4.4 0-8-3.2-8-7.2 0-3.6 2.8-6.6 6.6-7.1C18.1 15.4 21.8 13 26 13c5.1 0 9.3 3.5 10.4 8.3 4.6.2 8.3 3.6 8.3 7.9 0 4.4-4.1 8-9.1 8H18z',
          fill: 'rgba(255,255,255,0.92)',
          stroke: 'rgba(56,189,248,0.65)',
          strokeWidth: '1.5',
        }),
        jsx('path', {
          d: 'M28 21c2.3 0 4.2 1.9 4.2 4.2 0 1.6-1 3.1-2.4 3.8l6.5 10.2c-2.6 1.9-5.6 3-8.3 3s-5.7-1.1-8.3-3l6.5-10.2c-1.4-.7-2.4-2.2-2.4-3.8 0-2.3 1.9-4.2 4.2-4.2z',
          fill: 'rgba(34,197,94,0.22)',
          stroke: 'rgba(34,197,94,0.65)',
          strokeWidth: '1.5',
        }),
        jsx('path', {
          d: 'M44 14l1.5 3.4L49 19l-3.5 1.6L44 24l-1.5-3.4L39 19l3.5-1.6L44 14z',
          fill: 'rgba(250,204,21,0.95)',
        }),
      ],
    }),
  });

export const skyJourneyCampaign: Campaign = {
  id: 'journey',
  name: skyCotlCampaignStrings.journey.name,
  label: skyCotlCampaignStrings.journey.label,
  icon: SkyJourneyIcon,
  theme,
  questions,
};
