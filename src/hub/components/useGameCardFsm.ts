...
export type GameCardFsmState = 'Appear' | 'Idle' | 'Hover' | 'Press' | 'Ease';
...
export function useGameCardFsm({
  ...
  const scheduleEaseDone = useCallback(
    (next: GameCardFsmState) => {
      if (easeTimerRef.current) window.clearTimeout(easeTimerRef.current);
  ...
  }, [interactive, scheduleEaseDone, setFsmState, setIsOverState, startLoop]);
