...
vi.mock('@engine/utils/paths', async (importOriginal) => {
  const actual = await importOriginal<typeof import('@engine/utils/paths')>();
  return {
    ...actual,
  ...
  it('uses game-card when manifest declares it and updates load state', async () => {
    mockManifest({
      games: {
        poc: {
          cardAssets: {
            gameCard: '/games/poc/icons/game-card.webp',
            favicon: '/games/poc/favicon/favicon.svg',
          },
        },
      },
    });
    ...
    await waitFor(() => {
      expect(result.current.imageSrc).toContain(
        'games/poc/icons/game-card.webp',
      );
    });
