...
import type { BaseTheme } from '@engine/types';
...
export function LoadingProvider({ children }: { children: ReactNode }) {
  ...
  const hasIndeterminate = useMemo(() => {
    return Object.values(phases).some(
      (phase) =>
        phase.enabled && phase.status === 'loading' && phase.indeterminate,
    );
  }, [phases]);
  ...
  const phaseProgress = useMemo(() => {
    ...
    (Object.keys(phases) as LoadingPhase[]).forEach((phase) => {
      ...
      if (state.indeterminate) {
        ...
        const auto =
          duration > 0 ? Math.min(cap, (elapsed / duration) * cap) : 0;
  ...
  const totalWeight = useMemo(() => {
    return (Object.keys(PHASE_WEIGHTS) as LoadingPhase[]).reduce(
      (sum, phase) =>
        phases[phase].enabled ? sum + PHASE_WEIGHTS[phase] : sum,
      0,
    );
  }, [phases]);
  ...
  const setPhaseEnabled = useCallback(
    (phase: LoadingPhase, enabled: boolean) => {
      let didChange = false;
      setPhases((prev) => {
  ...
  const startPhase = useCallback((phase: LoadingPhase) => {
    ...
    traceLoading('loading-phase:start', { phase });
    setPhases((prev) => ({
      ...prev,
      [phase]: {
        ...prev[phase],
  ...
  const setPhaseProgress = useCallback((phase: LoadingPhase, next: number) => {
    const startedAt = performance.now();
    traceLoading('loading-phase:progress', {
      phase,
      progress: Math.round(clamp(next, 0, 1) * 1000) / 10,
    });
    setPhases((prev) => ({
      ...prev,
      [phase]: {
  ...
  const completePhase = useCallback((phase: LoadingPhase) => {
    traceLoading('loading-phase:end', { phase });
    setPhases((prev) => ({
      ...prev,
      [phase]: {
        ...prev[phase],
