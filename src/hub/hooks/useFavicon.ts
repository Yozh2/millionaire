...
import {
  createEmojiFavicon,
  DEFAULT_ENGINE_EMOJI,
} from '@hub/utils/emojiFavicon';
 ...
 */
async function findAppleTouchIcon(basePaths: string[]): Promise<string | null> {
  for (const basePath of basePaths) {
...
async function resolveHeadFaviconTags(
  ...
  const hasAnyFileIcon = Boolean(
    iconSvg || iconPng96 || iconPngLegacy || iconIco,
  );
  if (!hasAnyFileIcon) {
...
export async function resolveGameIcon(
  ...
  // 2) Try game emoji from manifest
  if (gameEmoji) {
 ...
 */
export function useFavicon(gameId: string | null, gameEmoji?: string): void {
...
export function useGameIcon(
  ...
  useEffect(() => {
    const nextIcon =
      preferredIconUrl ?? getCachedGameIcon(gameId, gameEmoji) ?? null;
