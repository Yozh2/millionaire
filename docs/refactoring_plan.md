...
Дата: 2025‑12‑13  
Статус: in progress (Этапы 1–10 ✅, 11+ запланированы)
...
- ⬜ **Этап 11. Engine UI: кнопки как компоненты**
  ...
  - Формализовать состояния кнопки (Spawn/Idle/Hover/Press/Click/Ease/Kill) в одном контракте.
...
- ⬜ **Этап 12. Header v1: настоящий layout‑header (не Panel)**
  - Вынести header из панелей в `src/engine/ui/layout/header/*` как глобальный layout‑слой.
