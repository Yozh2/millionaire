...
Дата: 2025‑12‑15
Статус: almost done (Этапы 1–17 ✅; осталось: Этап 18 ⬜ и merge в `main`)
...
- ✅ **Этап 17. Sound consent overlay (Headphones prompt)**
  - После `LoadingScreen` (Level 1) показать оверлей поверх всего: “лучше играть в наушниках… включить звук?”
  - Оверлей показывается **только при первой загрузке игры в рамках сессии** (не показывать на `NEW_GAME`/рестартах).
  - Две кнопки как `LifelineButton`: “со звуком” / “без звука” с иконками громкости.
  - Текст/лейблы кастомизируются через `GameConfig.systemStrings`.
  - Убрать принудительное включение звука по клику на CampaignCard.
  - Acceptance:
    - до выбора пользователем звук не включается автоматически;
