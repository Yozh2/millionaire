...
Дата: 2025‑12‑13
Статус: in progress (Этапы 1–11 ✅, 12 в работе, 13+ запланированы)
...
- `src/styles/buttons.css` — логика состояний кнопок + эффекты; цель: формализовать состояния (Appear/Idle/Hover/Press/Ease/Activate/Disappear). (`glare` уже в актуальном нейминге.)
...
- 2025‑12‑13 (codex): Этап 12 (частично): `HeaderPanel` вынесен в общий layout (`src/engine/ui/MillionaireGame.tsx`), удалены дубли из `StartScreen/GameScreen/EndScreen`, screen-transition класс поднят на layout.
